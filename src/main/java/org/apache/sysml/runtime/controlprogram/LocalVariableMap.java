...
public class LocalVariableMap implements Cloneable
{
	...
	public List<String> getCleanupDisabledVars() {
		return localMap.keySet().stream().filter(key -> localMap.get(key) instanceof CacheableData && !((CacheableData) localMap.get(key)).isCleanupEnabled())
